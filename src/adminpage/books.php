...
<head>
    ...
    <title>E-Library | Admin Books</title>
...
<body>
    ...
    <svg style="display:none;">
      ...
      <div id="content-header">
        <div class="content-cover">
          ...
          <div class="content-notification">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style="fill: rgba(0, 0, 0, 1);" class="darkmode">
      ...
      <div id="dashboard">
        <div class="upload_book">
          <h1>Upload Books</h1>
          <form method="post" class="book-upload-form" enctype="multipart/form-data">
                <input name="book_isbn" type="text" placeholder="ISBN" required spellcheck="false" autocomplete="off">
                <input name="book_name" type="text" placeholder="Name" required spellcheck="false" autocomplete="off">
                <input name="book_author" type="text" placeholder="Author" required spellcheck="false" autocomplete="off">
                <input name="book_language" type="text" placeholder="Language" required spellcheck="false" autocomplete="off">
                <input name="book_genre" type="text" placeholder="Genre" required spellcheck="false" autocomplete="off">
                <input name="book_published" type="date" placeholder="Published Date" required spellcheck="false" autocomplete="off">
                <h3>
                    BookFile
                </h3>
                <input name="book_file" type="file" required>
                <h3>
                    ImageFile
                </h3>
                <input name="book_image_file" type="file" required>
  ...
  if(isset($_POST['upload'])){
    ...
    $book_file = file_get_contents($_FILES['book_file']['tmp_name']);

    $stmt = mysqli_prepare($db, "INSERT INTO `books` (`isbn`, `name`, `author`, `language`, `genre`, `published`, `file`) VALUES (?, ?, ?, ?, ?, ?, ?)");
    mysqli_stmt_bind_param($stmt, "sssssss", $book_isbn, $book_name, $book_author, $book_language, $book_genre, $book_published, $book_file);
    ...
    $imagename = $_FILES['book_image_file']['name'];
    $imagefile = file_get_contents($_FILES['book_image_file']['tmp_name']);
    $stmt = mysqli_prepare($db, "INSERT INTO `book_images` (`book_id`, `imagename`, `imagefile`) VALUES (?, ?, ?)");
    mysqli_stmt_bind_param($stmt, "iss", $book_id, $imagename, $imagefile);
    mysqli_stmt_execute($stmt);
    // Close the prepared statement
    mysqli_stmt_close($stmt);
  }
?>
        ...
        <?php
          ...
          //   // $variable_queries = "INSERT INTO `books` VALUES ('','$_POST[book_isbn]','$_POST[book_name]','$_POST[book_author]','$_POST[book_language]','$_POST[book_genre]','$_POST[book_published]','$_POST[book_file]')";
          //   // $book_query = mysqli_query($db,$variable_queries);
            
          //   $stmt = $db->prepare("INSERT INTO books VALUES (?, ?, ?, ?, ?, ?, ?)");
          //   $stmt->bind_param("sssssss", $id, $_POST['book_isbn'], $_POST['book_name'], $_POST['book_author'], $_POST['book_language'], $_POST['book_genre'], $_POST['book_published'], $_POST['book_file']);
          //   $stmt->execute();
          ...
          //   $stmt = mysqli_prepare($db, "INSERT INTO `books` (`imagename`, `imagefile`) VALUES (?, ?)");
          //   mysqli_stmt_bind_param($stmt, "ss", $imagename, $imagefile);
          //   mysqli_stmt_execute($stmt);
          ...
          //   //  $stmt = mysqli_query($db, "INSERT INTO `books` VALUES ('','$_FILES[book_isbn]','$_FILES[book_name]','$_FILES[book_author]','$_FILES[book_language]','$_FILES[book_genre]','$_FILES[book_published]','$_FILES[book_file]',$_FILES['book_image_file'], $imagefile);");
  ...
  <script src="./adminscripts/notes.js" type="text/javascript"></script>
</body>
</html>